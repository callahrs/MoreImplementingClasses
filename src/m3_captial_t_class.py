...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        self.corner1 = rg.Point(intersection_center.x - (width / 2),
                                intersection_center.y - letter_thickness / 2)
        self.corner2 = rg.Point(intersection_center.x + (width / 2),
                                intersection_center.y + letter_thickness / 2)
        self.h_rect = rg.Rectangle(self.corner1, self.corner2)

        self.corner3 = rg.Point(
            intersection_center.x - (letter_thickness / 2),
            intersection_center.y - letter_thickness / 2)
        self.corner4 = rg.Point(
            intersection_center.x + (letter_thickness / 2),
            intersection_center.y + height)
        self.v_rect = rg.Rectangle(self.corner3, self.corner4)
    ...
    def clone(self):
        ...
        temp = CapitalT(self.intersection_center, self.width, self.height,
                        self.letter_thickness)
        temp.set_colors(self.fill_color, self.outline_color)
        return temp
